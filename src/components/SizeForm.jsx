...
import { sizeSchema } from "../validations/rules";
...
function SizeForm({ onSubmit, defaultValues, onCancel }) {
  ...
  const handleFormSubmit = async (event) => {
    ...
    const result = sizeSchema.safeParse({ name });
    if (!result.success) {
  ...
  return (
    <form onSubmit={handleFormSubmit} className="space-y-5">
      <Input
        ...
        placeholder="corto, medio, grande, etc."
        value={name}
