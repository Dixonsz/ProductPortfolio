...
import { stateSchema } from "../validations/rules";
...
function StateForm({ onSubmit, defaultValues, onCancel }) {
  ...
  const handleFormSubmit = async (event) => {
    ...
    const result = stateSchema.safeParse({ name });
    if (!result.success) {
  ...
  return (
    <form onSubmit={handleFormSubmit} className="space-y-5">
      <Input
        ...
        placeholder="Ejemplo: Nuevo, Usado, etc."
        value={name}
